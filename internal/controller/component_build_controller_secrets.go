...
// ensureNamespacePullSecret ensures that namespace pull secret exists and is linked to SA
func (r *ComponentBuildReconciler) ensureNamespacePullSecret(ctx context.Context, namespace string) error {
	log := ctrllog.FromContext(ctx)
	namespacePullSecretName := getNamespacePullSecretName(namespace)
...
// getNamespacePullSecretName returns name for the namespace pull dockerconfigjson secret
func getNamespacePullSecretName(namespaceName string) string {
	return fmt.Sprintf("%s-namespace-pull", namespaceName)
}
