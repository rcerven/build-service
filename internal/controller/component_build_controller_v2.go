	...
	if len(versionsToTriggerBuildFor) > 0 {
		...
		for _, versionName := range versionsToTriggerBuildFor {
			...
			if err != nil {
				...
				// Then, remove successfully triggered builds from actions (priority operation over error above)
				if len(successfullyTriggered) > 0 {
			...
			for _, versionName := range versionsToOffboard {
				...
				if err != nil {
					...
					if errors.As(err, &boErr) {
						isPersistent = boErr.IsPersistent()
					...
					// Remove successfully offboarded versions from status (priority operation over error above)
	...
	if len(versionsToCreatePRFor) > 0 {
		...
		for _, versionName := range versionsToCreatePRFor {
			...
			if err != nil {
				...
				// Remove successfully created configurations from actions
				if len(successfulWithPRs) > 0 {
					successfulVersionNames := make([]string, 0, len(successfulWithPRs))
					for _, v := range successfulWithPRs {
						successfulVersionNames = append(successfulVersionNames, v.name)
					}
					if removeErr := r.removeCreateConfigurationActions(ctx, &component, successfulVersionNames, false, versionsToCreatePRFor); removeErr != nil {
						log.Error(removeErr, "Failed to remove create configuration actions", "VersionsWithPRs", successfulVersionNames)
						return ctrl.Result{}, removeErr
					}
					// Successfully removed actions - this triggered a new reconcile, so return success
					// to avoid double reconcile
...
// - singleVersion: single version name to target (ignored if allVersions is true)
// - multipleVersions: array of version names to target (ignored if allVersions is true)
